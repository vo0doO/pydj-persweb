...
class AuthenticationBackend(ModelBackend):
    ...
    def _authenticate_by_email(self, **credentials):
        # Even though authentication will pass along `email`, other apps may
    ...
    def _stash_user(cls, user):
        ...
        checks `is_active`. Now, earlier versions of authentication did not do this
        and simply returned the user as authenticated, even in case of
        `is_active=False`. For authentication scope, this does not pose a problem, as
        these users are properly redirected to an account inactive page.

        This does pose a problem when the authentication backend is used in a
        different context where authentication is not responsible for the login. Then,
        ...
        In order to preserve the authentication behavior while respecting Django's
        logic, we stash a user for which the password check succeeded but
        `user_can_authenticate()` failed. In the authentication authentication logic,
