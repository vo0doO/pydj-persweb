...
from authentication.account.forms import BaseSignupForm, SignupForm
from authentication.account.models import (
    EmailAddress,
    EmailConfirmation,
    EmailConfirmationHMAC,
)
from authentication.tests import Mock, TestCase, patch
from authentication.utils import get_user_model, get_username_max_length
...
@override_settings(
    ...
    ACCOUNT_ADAPTER='authentication.account.adapter.DefaultAccountAdapter',
    ACCOUNT_USERNAME_REQUIRED=True)
class AccountTests(TestCase):
    def setUp(self):
        if 'authentication.socialaccount' in settings.INSTALLED_APPS:
    ...
    @override_settings(
        ACCOUNT_USERNAME_VALIDATORS='authentication.account.tests'
        '.test_username_validators')
...
class UtilsTests(TestCase):
    ...
    def test_url_str_to_pk_identifies_UUID_as_stringlike(self):
        with patch('authentication.account.utils.get_user_model') as mocked_gum:
...
class ConfirmationViewTests(TestCase):
    ...
    @override_settings(ACCOUNT_EMAIL_CONFIRMATION_HMAC=True,
                       ACCOUNT_LOGIN_ON_EMAIL_CONFIRMATION=True)
    @patch('authentication.account.views.perform_login')
    @patch('authentication.account.utils.get_user_model', return_value=UUIDUser)
    def test_login_on_confirm_uuid_user(self, mocked_gum, mock_perform_login):
        ...
        with patch('authentication.account.views.EmailConfirmationHMAC',
                   confirmation):
            self.client.post(
                reverse('account_confirm_email',
                        args=[key]))
